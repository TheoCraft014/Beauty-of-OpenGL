...
void gfxc::CameraInput::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->KeyHold(GLFW_KEY_KP_4))         camera->RotateOY( 400 * deltaTime);
    if (window->KeyHold(GLFW_KEY_KP_6))         camera->RotateOY(-400 * deltaTime);
    if (window->KeyHold(GLFW_KEY_KP_8))         camera->RotateOX( 600 * deltaTime);
    if (window->KeyHold(GLFW_KEY_KP_5))         camera->RotateOX(-600 * deltaTime);
